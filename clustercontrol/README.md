...
Installs ClusterControl on your existing database cluster. ClusterControl is an agentless operational management and automation software for database clusters which aims to simplify development, monitoring management and scaling of clusters. 

Supported database cluster:
...
The ClusterControl module manages and configures ClusterControl and all of its components and requirements particularly:
* Install ClusterControl controller, cmonapi and UI via Severalnines package repository.
* Install and configure MySQL, ceate cmon DB, grant cmon user and configure DB for ClusterControl UI.
...
This module will install ClusterControl community edition. You are welcome to request a free two-week trial license via our [contact us](http://www.severalnines.com/contact-us) page or email us at info@severalnines.com.
...
Make sure you meet following criterias prior to the deployment:
* ClusterControl node must run on a clean dedicated host and has internet connection.
...
* SELinux/AppArmor should be turned off. Services or ports to be enabled are listed [here](http://www.severalnines.com/docs/clustercontrol-administration-guide/administration/securing-clustercontrol).
...
Following options are used for general ClusterControl set up:

####`is_controller`
Define whether the node is ClusterControl controller host. All nodes that you want ClusterControl to monitor should be set to false. 
Default: true

####`controller_ip_address`
Specify the IP address of ClusterControl node. Only specify this setting on nodes that you want ClusterControl to monitor. 
...
####`ssh_user`
Specify the SSH user that ClusterControl will used for management and automation process. Unless root, make sure this user is in sudoers list. 
Default: 'root'

####`sudo_password`
If sudo user has password, specify it here. ClusterControl requires this to automate database recovery, management and deployment. If `ssh_user` is root, this will be ignored. 
...
####`mysql_cmon_password`
Specify the MySQL password for user cmon. This module will grant this user with specified password for ClusterControl usage. 
Default: 'cmon'
...
####`datadir`
Comma-separated list of database's data directory that should be monitored by ClusterControl. MySQL is equal to datadir while MongoBD is equal to dbpath.
...
Comma-separated list of IP address/hostname with port of MongoDB/TokuMX shard/replica set node that ClusterControl should monitor. 
...
Please report bug or suggestion via our support channel: [https://support.severalnines.com](https://support.severalnines.com)
